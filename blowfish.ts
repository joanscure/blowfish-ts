...
const isString = function (it: string | any) {
...
arrayUtil.map = function (
  arr: any[] | string,
  callback: any | string | number,
  thisObject: any,
  Ctr: any
) {
  var i = 0,
...
function init(key: any) {
  var k = key;
  if (isString(k)) {
    k = arrayUtil.map(k.split(""), function (item: any) {
  ...
  var pos = 0,
    ...
    l;
  var box: any = {
    p: arrayUtil.map(boxes.p.slice(0), function (item: any) {
      var l = k.length,
        j;
      for (j = 0; j < 4; j++) {
...
const Blowfish = {
  ...
  getIV: (outputType?: TYPE) => {
    var out = outputType || TYPE.BASE64;
    switch (out) {
      case TYPE.HEX: {
        return arrayUtil
          .map(iv, function (item: any) {
  ...
  setIV: function (data: string, inputType?: TYPE) {
    var ip = inputType || TYPE.BASE64;
    var ba = null;
    switch (ip) {
      case TYPE.STRING: {
        ba = arrayUtil.map(data.split(""), function (item: any) {
  ...
  decrypt: function (
    ...
    switch (ip) {
      ...
      case TYPE.STRING: {
        c = arrayUtil.map(ciphertext.split(""), function (item: any) {
          return item.charCodeAt(0);
        });
        break;
      }
